...
OpenSun()
{
    ...
    fdSun = open(XINU_CONSOLE, O_RDWR, 0);

    ioctl(fdSun, TIOCGETP, &tty_mode);
    
    copy_sgtty(mode, tty_mode);
    mode.sg_flags &= ~(TANDEM|CBREAK|ECHO /*|CRMOD*/);
    mode.sg_flags |= RAW | CRMOD;
    ioctl(fdSun, TIOCSETP, &mode);

    /* flush the Q */
    arg = 0;
    ioctl(fdSun, TIOCFLUSH, &arg);

    if (verbose)
	printf("Sending break\n");

    /* make sure we have it's attention */
    ioctl(fdSun, TIOCSBRK, NULL);
    usleep(BREAK_TIME);
    ioctl(fdSun, TIOCCBRK, NULL);
